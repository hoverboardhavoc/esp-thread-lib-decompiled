...
/* ot::Spinel::RadioSpinel<esp::openthread::UartSpinelInterface,
   esp_openthread_mainloop_context_t>::GetTransmitPower(signed char&) */

int ot::Spinel::RadioSpinel<esp::openthread::UartSpinelInterface,esp_openthread_mainloop_context_t>
    ::GetTransmitPower(signed *param_1)

{
  int iVar1;
  undefined4 uVar2;
  
  iVar1 = Get((ulong)param_1,(char *)0x25);
  if (iVar1 != 0) {
    uVar2 = otThreadErrorToString();
    otLogWarn(0xc,_LC3,"%s: %s","Get transmit power failed",uVar2);
  }
  return iVar1;
}
