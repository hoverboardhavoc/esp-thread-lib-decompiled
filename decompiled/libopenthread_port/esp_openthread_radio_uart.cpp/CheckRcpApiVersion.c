...

{
  ...
  if ((CONCAT31(in_register_0000202d,param_1) == 0) ||
     (iVar1 = Get((ulong)this,(char *)0xb0,&_LC10,aiStack_14), iVar1 == 0)) {
    _otLogNote(0xc,"RCP API Version: %u",aiStack_14[0]);
    iVar1 = 0;
    if (aiStack_14[0] != 4) {
      _otLogCrit(0xc,"RCP API Version %u is not in the supported range [%u-%u]",4,4);
      uVar2 = otExitCodeToString(3);
      _otLogCrit(0xc,"%s() at %s:%d: %s","CheckRcpApiVersion",
