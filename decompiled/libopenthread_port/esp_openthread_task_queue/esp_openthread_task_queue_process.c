...

{
  ...
  if (((s_task_queue_event_fd < 0x40) &&
      ((1 << (s_task_queue_event_fd & 0x1f) & *(uint *)(param_1 + (s_task_queue_event_fd >> 5) * 4))
       != 0)) && (sVar1 = read(s_task_queue_event_fd,&pcStack_18,8), sVar1 != 8)) {
    __assert_func(0,0,0,0);
  }
  if (s_task_queue == 0) {
    uVar2 = esp_log_timestamp();
    esp_log_write(1,"OPENTHREAD",&_LC1,uVar2,"OPENTHREAD","esp_openthread_task_queue_process",0x55);
    uVar2 = 0x103;
  }
  else {
