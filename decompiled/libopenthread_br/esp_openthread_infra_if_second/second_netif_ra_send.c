...

{
  ...
  uVar2 = otThreadGetDeviceRole();
  esp_openthread_task_switching_lock_release();
  iVar3 = nd6_create_ra_with_hdr(0x68);
  if (iVar3 == 0) {
    uVar1 = esp_log_timestamp();
    esp_log(1,"OPENTHREAD","E (%lu) %s: %s(%d): Failed to create header for br second netif ra\n",
            uVar1,"OPENTHREAD","br_second_netif_create_ra",0x4a);
  }
  else {
    memcpy(auStack_34,s_netif_prefix,0x10);
    memcpy(auStack_80,auStack_34,0x14);
    nd6_append_option_pio(iVar3,auStack_80,s_netif_prefix[0x10],0x708);
    if ((1 < uVar2) || (param_1 == 3)) {
      esp_openthread_task_switching_lock_acquire(0xffffffff);
      iVar4 = otBorderRoutingGetOnLinkPrefix(uVar1,auStack_5c);
      iVar5 = otBorderRoutingGetOmrPrefix(uVar1,&uStack_48);
      esp_openthread_task_switching_lock_release();
      if (iVar4 == 0) {
        memcpy(auStack_a0,auStack_5c,0x11);
        append_rio_to_ra(iVar3,auStack_a0,1 < uVar2);
      }
      if (iVar5 == 0) {
    ...
    uStack_48 = 0;
    uStack_44 = 0;
    iVar4 = esp_netif_get_mac(&uStack_48);
    if (iVar4 == 0) {
      nd6_append_option_sll(iVar3,&uStack_48);
      ip6addr_aton("FF02::1",auStack_34);
      memcpy(auStack_80,auStack_34,0x14);
      iVar4 = icmp6_raw_send(s_raw_pcb,auStack_80,iVar3,1);
      if (iVar4 != 0) {
        uVar1 = esp_log_timestamp();
    ...
    esp_log(1,"OPENTHREAD","E (%lu) %s: %s(%d): Failed to get mac addr of the second netif\n",uVar1,
            "OPENTHREAD","br_second_netif_create_ra",0x6b);
    pbuf_free(iVar3);
  }
  uVar1 = esp_log_timestamp();
  esp_log(1,"OPENTHREAD","E (%lu) %s: %s(%d): Failed to create RA for the second netif\n",uVar1,
          "OPENTHREAD","create_and_send_second_netif_ra",0x7e);
_L0:
  ...
  if (s_netif_ra_enabled != '\0') {
    ...
    sys_timeout(second_netif_ra_send,uVar1);
    return;
  }
  return;
}
